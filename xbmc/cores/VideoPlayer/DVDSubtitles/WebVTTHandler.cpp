...
bool WebVTTHandler::Initialize()
{
  ...
  // Load CSS colors
  //! @todo change this with a different color loader
  // colors have to be loaded only when needed (lazy way)
...
void WebVTTHandler::DecodeLine(std::string line, std::vector<subtitleData>* subList)
{
  ...
  else if (m_currentSection == WEBVTT_SECTION_STYLE)
  {
    ...
    if (!m_cueCurrentCssStyleSelectors.empty())
    {
      ...
      {
        //! @todo Make a lazy color list loading, then convert the CSS color
        //uint32_t color = ColorUtils::ConvertToBGR(m_colorManager.GetColor(colorName));
        uint32_t color = UTILS::COLOR::BLACK; // Remove me
      ...
      if (!colorRGB.empty()) // From CSS Color numeric R,G,B values
      {
        ...
        uint32_t color = UTILS::ConvertIntToRGB(std::stoi(intValues[2]), std::stoi(intValues[1]),
                                                std::stoi(intValues[0]));
        m_feedCssStyle.color = StringUtils::Format("{:6x}", color);
      }
      // Font bold
