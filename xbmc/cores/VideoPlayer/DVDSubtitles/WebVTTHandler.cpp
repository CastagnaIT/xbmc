...
// Some features are partially supported (CSS parsing)
// Currently unsupported features:
// - Cue region (text will be displayed at the bottom)
// - Alignment property of Cue "line" setting (uncompatible)
...
{
  if (strlen(data) > strlen(signature))
  {
    if (strncmp(data, signature, strlen(signature)) == 0)
    {
      // Check if last char is valid
      if (strchr(signatureLastChars, data[strlen(signature)]) != NULL)
        return true;
    }
  }
  return false;
...
std::string ConvertStyleToCloseTags(int flagTags[], webvttCssStyle& style, webvttCssStyle& baseStyle)
{
  std::string tags;
  if (style.isFontBold)
  {
  ...
  if (style.isFontUnderline)
  {
    flagTags[FLAG_TAG_UNDERLINE] = flagTags[FLAG_TAG_UNDERLINE] > 0 ? --flagTags[FLAG_TAG_UNDERLINE] : 0;
    if (flagTags[FLAG_TAG_UNDERLINE] == 0)
      tags += "{\\u0}";
  }
  if (!style.color.empty())
...
bool WebVTTHandler::Initialize()
{
  m_currentSection = WEBVTT_SECTION_UNDEFINED;

  // Load CSS colors
  m_colorManager.Load("SKINDEFAULT");
...
bool WebVTTHandler::CheckSignature(const char* buffer)
{
...
void WebVTTHandler::DecodeLine(std::string line, std::vector<subtitleData>* subList)
{
  ...
  if (m_currentSection == WEBVTT_SECTION_UNDEFINED)
  {
    if (line == "STYLE" || line == "Style:") // "Style:" Youtube spec
    {
      m_currentSection = WEBVTT_SECTION_STYLE;
      return;
    }
    else if (line == "REGION")
    {
      m_currentSection = WEBVTT_SECTION_REGION;
      return;
    }
    else if (line == "NOTE")
    {
      m_currentSection = WEBVTT_SECTION_NOTE;
    }
    ...
    else if (IsCueLine(line))
    {
      ...
      m_currentSection = WEBVTT_SECTION_CUE;
    }
  }

  if (m_currentSection == WEBVTT_SECTION_CUE || m_currentSection == WEBVTT_SECTION_CUE_TEXT)
  {
    if (IsCueLine(line))
    {
      if (m_currentSection == WEBVTT_SECTION_CUE_TEXT && !m_subtitleData.text.empty())
      {
        CLog::Log(LOGWARNING,
        ...
        subList->push_back(m_subtitleData);

        // Change to a new cue section
        m_currentSection = WEBVTT_SECTION_CUE;
      }
      else if (m_currentSection == WEBVTT_SECTION_CUE_TEXT)
      {
      ...
      if (m_currentSection == WEBVTT_SECTION_CUE)
      ...
      m_currentSection = WEBVTT_SECTION_CUE_TEXT;
      return; // Start read the cue text area from the next line
    }

    if (m_currentSection == WEBVTT_SECTION_CUE_TEXT)
    {
      ...
      else
      {
        if (line == "{")
        {
          ...
          m_currentSection = WEBVTT_SECTION_CUE_METADATA;
          return;
        }
        ...
        if (m_subtitleData.text.empty())
          CalculateTextPosition(line, m_subtitleData);

        m_subtitleData.text += line;
      }
    }
  }
  else if (m_currentSection == WEBVTT_SECTION_CUE_METADATA)
  {
    // Cue metadata is not supported (is for scripted apps)
    if (line.empty()) // End of section
      m_currentSection = WEBVTT_SECTION_UNDEFINED;
  }
  else if (m_currentSection == WEBVTT_SECTION_STYLE)
  {
    ...
    {
      std::string selectorName = m_cueCssTagRegex.GetMatch(1); // TODO: unescape chars
      m_cueCurrentCssStyleSelectors.push_back(selectorName);
    ...
    if (!m_cueCurrentCssStyleSelectors.empty())
    {
      ...
      if (m_feedCssStyle.selectorName.empty())
        m_feedCssStyle.selectorType = WEBVTT_SELECTOR_ANY;
      else if (m_feedCssStyle.selectorName.compare(0, 1, "#") == 0)
        m_feedCssStyle.selectorType = WEBVTT_SELECTOR_ID;
      else if (m_feedCssStyle.selectorName.find(".") != std::string::npos)
        m_feedCssStyle.selectorType = WEBVTT_SELECTOR_CLASS;
      else if (m_feedCssStyle.selectorName.find("[") != std::string::npos) // Attribute selector not implemented
        m_feedCssStyle.selectorType = WEBVTT_SELECTOR_UNSUPPORTED;
      else if (m_feedCssStyle.selectorName.compare(0, 1, ":") == 0) // Pseudo-classes not implemented
        m_feedCssStyle.selectorType = WEBVTT_SELECTOR_UNSUPPORTED;
      else
        m_feedCssStyle.selectorType = WEBVTT_SELECTOR_TYPE;

      if (m_feedCssStyle.selectorType == WEBVTT_SELECTOR_UNSUPPORTED)
      {
      ...
      {
        uint32_t color = ColorUtils::ConvertToBGR(m_colorManager.GetColor(colorName));
      ...
      if (!colorRGB.empty()) // From CSS Color numeric R,G,B values
      {
        ...
        uint32_t color = ColorUtils::ConvertIntToRGB(
    ...
    if (line == "}" || line.empty()) // End of current style
    {
      ...
      {
        // Store the style
        m_cueCssStyles.push_back(m_feedCssStyle);

        if (m_feedCssStyle.selectorType == WEBVTT_SELECTOR_TYPE)
        {
          ...
          for (int i = 1; i < m_cueCurrentCssStyleSelectors.size(); i++)
          {
            ...
            m_cueCssStyles.push_back(cssStyleCopy);
          }
        }
        m_cueCurrentCssStyleSelectors.clear();
      }
  ...
  else if (m_currentSection == WEBVTT_SECTION_REGION)
  {
    // Regions are not supported
    if (line.empty()) // End of section
      m_currentSection = WEBVTT_SECTION_UNDEFINED;
  }
  else if (m_currentSection == WEBVTT_SECTION_NOTE)
  {
    if (line.empty()) // End of section
      m_currentSection = WEBVTT_SECTION_UNDEFINED;
  }
...
bool WebVTTHandler::IsCueLine(std::string& line)
{
  return m_cueTimeRegex.RegFind(line) >= 0;
}

void WebVTTHandler::GetCueData(std::string& cueText)
{
  ...
  if (m_cueTimeRegex.GetSubCount() == 6)
  {
    ...
    m_subtitleData.startTime = double(sHours * 3600 + sMinutes * 60) + sSeconds + m_mpegTsOffset;
    m_subtitleData.stopTime = double(eHours * 3600 + eMinutes * 60) + eSeconds + m_mpegTsOffset;
    cueSettings =
  ...
  GetCueSettings(cueSettings, m_subtitleData);
}

void WebVTTHandler::GetCueSettings(std::string& cueSettings, subtitleData& subData)
{
  webvttCueSettings settings;
  // settings.regionId = ""; // "region" is not supported yet

  std::string cueVertical = GetCueSettingValue("vertical", cueSettings, ""); // Ref. Writing direction
  if (cueVertical == "lr")
    settings.verticalAlign = WEBVTT_VALIGN_VERTICAL_LR;
  else if (cueVertical == "rl")
    settings.verticalAlign = WEBVTT_VALIGN_VERTICAL_RL;
  else
    settings.verticalAlign = WEBVTT_VALIGN_HORIZONTAL;
  ...
  if (cuePos == "left")
    settings.positionAlign = WEBVTT_ALIGN_LEFT;
  else if (cuePos == "right")
    settings.positionAlign = WEBVTT_ALIGN_RIGHT;
  else if (cuePos == "center" || cuePos == "middle") // "middle" undocumented
    settings.positionAlign = WEBVTT_ALIGN_CENTER;
  else if (cuePos == "start")
    settings.positionAlign = WEBVTT_ALIGN_START; // Undocumented
  else if (cuePos == "end")
    settings.positionAlign = WEBVTT_ALIGN_END; // Undocumented
  else 
    settings.positionAlign = WEBVTT_ALIGN_AUTO;
  ...
  if (cueAlign == "left")
    settings.align = WEBVTT_ALIGN_LEFT;
  else if (cueAlign == "right")
    settings.align = WEBVTT_ALIGN_RIGHT;
...
void WebVTTHandler::CalculateTextPosition(std::string& subtitleText, subtitleData& subData)
{
  // Here we cannot handle a kind of cue box,
  // to simulate it we use the margins to position the text on the screen 
  // the subtitle text alignment will be also computed

  // Sidenote: Limits to bidi direction checks,
  // The webvtt doc specifies that each line of text should be aligned according 
  // to the direction of text, but here we limit this by check the text
  // direction of the first line only, and will be used for all lines of the cue.
  ...
  webvttCueSettings* settings = &subData.cueSettings;

  // Compute cue box "align" value
  if (settings->align == WEBVTT_ALIGN_START)
  {
    if (CCharsetConverter::utf8IsRTLBidiDirection(subtitleText))
      settings->align = WEBVTT_ALIGN_RIGHT;
    else
      settings->align = WEBVTT_ALIGN_LEFT;
  }
  ...
  if (settings->position.isAuto)
  {
    ...
    if (settings->align == WEBVTT_ALIGN_LEFT)
      settings->position.value = 0;
    else if (settings->align == WEBVTT_ALIGN_RIGHT)
      settings->position.value = 100;
    else
      settings->position.value = 50;
  }
  int cuePosPixel = int((SUBTITLE_VIEWPORT_WIDTH / 100) * settings->position.value);

  // Compute cue box "position alignment" value
  if (settings->positionAlign == WEBVTT_ALIGN_AUTO)
  {
    if (settings->align == WEBVTT_ALIGN_LEFT)
      settings->positionAlign = WEBVTT_ALIGN_LEFT; // line-left
    else if (settings->align == WEBVTT_ALIGN_RIGHT)
      settings->positionAlign = WEBVTT_ALIGN_RIGHT; // line-right
    else
      settings->positionAlign = WEBVTT_ALIGN_CENTER;
  }
  ...
  int cueSizePixel = int((SUBTITLE_VIEWPORT_WIDTH / 100) * settings->size);
  
  // Calculate Left/Right margins,
  // by taking into account cue box "position alignment" and cue box "size"
  if (settings->positionAlign == WEBVTT_ALIGN_LEFT) // line-left
  {
    marginLeft = cuePosPixel;
    marginRight = int(SUBTITLE_VIEWPORT_WIDTH - (cuePosPixel + cueSizePixel));
  }
  else if (settings->positionAlign == WEBVTT_ALIGN_RIGHT) // line-right
  {
    marginLeft = int(cuePosPixel - cueSizePixel);
    marginRight = int(SUBTITLE_VIEWPORT_WIDTH - cuePosPixel);
  }
  else if (settings->positionAlign == WEBVTT_ALIGN_CENTER)
  {
    int cueHalfSize = int(cueSizePixel / 2);
    marginLeft = cuePosPixel - cueHalfSize;
    marginRight = int(SUBTITLE_VIEWPORT_WIDTH - (cuePosPixel + cueHalfSize));
  }
  ...
  {
    useAlignBottom = true;
    marginVertical = SUBTITLE_MARGIN_VERTICAL;
  }
  else
    marginVertical = int((SUBTITLE_VIEWPORT_HEIGHT / 100) * cueLinePerc);
  
  if (settings->align == WEBVTT_ALIGN_LEFT)
    textAlign = useAlignBottom ? ALIGN_SUB_LEFT : ALIGN_TOP_LEFT;
  else if (settings->align == WEBVTT_ALIGN_RIGHT)
    textAlign = useAlignBottom ? ALIGN_SUB_RIGHT : ALIGN_TOP_RIGHT;
  else
    textAlign = useAlignBottom ? ALIGN_SUB_CENTER : ALIGN_TOP_CENTER;

  subData.textAlign = textAlign;
  subData.useMargins = !m_overridePositions;
  subData.marginLeft = marginLeft;
  subData.marginRight = marginRight;
  subData.marginVertical = marginVertical;
}

std::string WebVTTHandler::GetCueSettingValue(std::string propName,
                                               std::string& text,
...
std::string WebVTTHandler::GetCueCssValue(std::string cssPropName, std::string& line)
{
...
void WebVTTHandler::AddDefaultCssClasses()
{
  ...
  for (auto& cClass : cueCssDefaultColorsClasses)
  {
    ...
    m_cueCssStyles.push_back(style);
  }
}

bool WebVTTHandler::GetBaseStyle(webvttCssStyle& style)
{
  // Get the style applied to all cue's (type WEBVTT_SELECTOR_ANY) and without name
  ...
  // Try find the CSS Style by cue ID (WEBVTT_SELECTOR_ID)
...
void WebVTTHandler::ConvertSubtitle(std::string& text)
{
  int pos = 0;
  ...
  {
    if (m_subtitleData.textAlign == ALIGN_TOP_LEFT)
      text.insert(0, "{\\an7}");
    else if (m_subtitleData.textAlign == ALIGN_TOP_RIGHT)
      text.insert(0, "{\\an9}");
    else if (m_subtitleData.textAlign == ALIGN_TOP_CENTER)
      text.insert(0, "{\\an8}");
    else if (m_subtitleData.textAlign == ALIGN_SUB_LEFT)
      text.insert(0, "{\\an1}");
    else if (m_subtitleData.textAlign == ALIGN_SUB_RIGHT)
      text.insert(0, "{\\an3}");
    else if (m_subtitleData.textAlign == ALIGN_SUB_CENTER)
      text.insert(0, "{\\an2}");
  }
}

void WebVTTHandler::InsertCssStyleStartTag(std::string& tagName,
                                           std::string& text,
                                           int& pos,
                                           int flagTags[],
                                           std::map<std::string, std::string>& cssTagsOpened)
...
void WebVTTHandler::InsertCssStyleCloseTag(std::string& tagName,
                                           std::string& text,
                                           int& pos,
                                           int flagTags[],
                                           std::map<std::string, std::string>& cssTagsOpened,
