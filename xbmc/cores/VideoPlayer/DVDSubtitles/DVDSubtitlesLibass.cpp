                                            ...
                                            int* changes)
{
  ...
  {
    CLog::Log(LOGERROR, "{} - ASS renderer/ASS track not initialized.",
              __FUNCTION__);
    return NULL;
  }
      ...
      (m_subtitleType == NATIVE && subStyle.assOverrideStyles != AssOverrideStyles::DISABLED))
  {
        ...
        (m_subtitleType == NATIVE && subStyle.assOverrideStyles != AssOverrideStyles::POSITIONS))
    {
      ...
      if (opts.usePosition)
        ...
        style->MarginV = SUBTITLE_MARGIN_VERTICAL * scale;
    }
...
int CDVDSubtitlesLibass::AddEvent(const char* text, double startTime, double stopTime, subtitleOpts* opts)
{
