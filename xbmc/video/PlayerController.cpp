  ...
  if (g_application.GetAppPlayer().IsPlayingVideo())
  {
    switch (action.GetID())
    {
      ...
      case ACTION_SUBTITLE_VSHIFT_UP:
      {
        ...
        double pos = g_application.GetAppPlayer().GetSubtitleShiftPos();

        pos -= m_movingSpeed.GetUpdatedDistance(ACTION_SUBTITLE_VSHIFT_UP);
        if (pos < resInfo.Overscan.top)
          pos = resInfo.Overscan.top;

        ShowSlider(action.GetID(), 277, static_cast<float>(pos),
                   static_cast<float>(resInfo.Overscan.top), 1.0f,
                   static_cast<float>(resInfo.Overscan.bottom));
        g_application.GetAppPlayer().SetSubtitleShiftPos(pos);
        return true;
      }
      ...
      case ACTION_SUBTITLE_VSHIFT_DOWN:
      {
        ...
        pos += m_movingSpeed.GetUpdatedDistance(ACTION_SUBTITLE_VSHIFT_DOWN);
        if (pos > static_cast<double>(resInfo.Overscan.bottom))
          pos = static_cast<double>(resInfo.Overscan.bottom);
...
void CPlayerController::OnSliderChange(void *data, CGUISliderControl *slider)
{
  ...
  {
    std::string strValue = StringUtils::Format("{:d}px", static_cast<int>(slider->GetFloatValue()));
    slider->SetTextValue(strValue);
  }
